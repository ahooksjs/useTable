...
const select1 = () => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        data: [
          { label: 'Select1默认选中', value: 'default_select1' },
          { label: 'Select1测试', value: 'test1' },
        ],
      });
    }, 1000);
  });
...
const Component = () => {
  const asyncDefaultPlugin = useAsyncDefaultPlugin({
  ...
  return (
    <Fragment>
      <SchemaForm {...formProps} components={{ Select }} style={{ marginBottom: 20 }} inline>
        ...
        <Field name="name1" title="name" x-component={'Select'} />
        <FormButtonGroup>
          <Submit>查询</Submit>
          <Reset>重置</Reset>
        </FormButtonGroup>
